...
<!-- <script src="http://code.jquery.com/jquery-1.9.1.min.js"></script>
   <script type="text/javascript">
        $(document).ready(function () {
            ...
            //[2] 자식 요소를 갖는 li에 대해서는 블릿이미지를 plus.gif를 지정
            $('.organ ul li') //자식 요소(ul)를 갖는 요소(li)에 대해서
                .css({cursor: 'pointer', 'list-style-image':'url(/NHMP/resources/ERP/images/folder-closed.gif)'})//+기호로 설정
                .children().hide(); //자식요소 숨기기
               
            //[3] +로 설정된 항목에 대해서 click이벤트 적용
            $('.organ ul li').click(function(event){
                      ...
                      if ($(this).children().is(':hidden')) {
                        ...
                        // 보이기
          				...
          				success : function(data){
          					...
          					for(var i in json.list){
          						var li = $("<ul><li class='teamList'>" + 
          								decodeURIComponent(json.list[i].teamname).replace(/\+/gi, " ") +  "</li></ul>");
          						$('.deptList').eq(index).append(li);
          					}
